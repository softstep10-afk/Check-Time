...
## 📌 Уточнение к X3 — важно, прочитать перед началом волны

Дискуссия 19.04.2026 после формулировки выше. Исходный текст X3 описывал flag как приватный канал менеджеру — **это неверно**. Андрей уточнил цель:

**Правильная цель:** публичный инцидент-трекер для команды на проекте. Все видят проблему, никто не видит кто её пометил.

**Правильный flow:**
1. Работник (любой на проекте) видит косяк на фото → жмёт «🚩 Отметить для проверки» + опционально пишет заметку «тут перекос / нужен материал / переделать»
2. **Вся команда проекта** теперь видит значок 🚩 + заметку на этом фото
3. **Имя flagged_by скрыто** от всех работников на уровне RLS
4. **Owner/Manager** видит и автора, и все флаги — может пометить review complete
5. Цель — **совместное устранение**, не наказание. Никто не «доносит», все «помечают для исправления»

**Техническая реализация (уточнённая):**

```sql
-- 00012_media_flags.sql
create table public.media_flags (
  id uuid primary key default gen_random_uuid(),
  media_id uuid references public.media(id) on delete cascade,
  flagged_by uuid references public.profiles(id),  -- ТОЛЬКО owner/manager видят
  note text,                                        -- видят ВСЕ на проекте
  flagged_at timestamptz not null default now(),
  reviewed_at timestamptz,
  reviewed_by uuid references public.profiles(id),
  reviewed_note text
...
-- Create a view for workers (without flagged_by)
create view public.media_flags_public as
select id, media_id, note, flagged_at, reviewed_at, reviewed_note
from public.media_flags;
```

**RLS:**
- Таблица `media_flags` доступна на SELECT **только owner/manager**
- View `media_flags_public` доступна всем, кто assigned на проект этого media
- INSERT в `media_flags` — любой работник на проекте, `flagged_by = auth.uid()` через RLS

**UI для работника:**
- На каждом фото проекта — маленький значок 🚩 (флаг, если есть хоть один для этого media)
- Клик на фото → видно все заметки («перекос», «нужен материал», «переделать — 2 дня назад»)
- Кнопка «🚩 Отметить для проверки» + textarea для заметки
- **Нет поля «автор»** и нигде не виден список кто-что-пометил

**UI для менеджера/owner:**
- То же самое + колонка «Кто пометил» + timestamp
- Может нажать «✅ Проверено» → `reviewed_at` проставляется, 🚩 снимается

**Результат для коллектива:**
- Проблема видна — можно обсудить и исправить
- Автор неизвестен — не будут «клевать»
- Менеджер при этом знает, кто инициативный — полезно для оценки

**Размер:** ~45 мин (как было). Миграция 00012. Риск: средний (новая таблица + view + RLS).
