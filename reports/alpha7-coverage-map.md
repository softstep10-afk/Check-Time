# Alpha-7 Coverage Map

Static local scan of tests, source, and docs for critical Alpha-7 invariants.

| Category | Coverage | Tests | Source | Recommendation |
| --- | --- | --- | --- | --- |
| material driver-only dropdown | covered | 2 | 1 | None |
| material task server validation | covered | 4 | 4 | None |
| driver role filtering | covered | 1 | 11 | None |
| private message sender/recipient persistence | covered | 4 | 11 | None |
| task read/taken/done visibility | covered | 5 | 16 | None |
| notification-as-signal | covered | 5 | 25 | None |
| mobile Поехать preference | covered | 3 | 3 | None |
| project navigation Apple/Google/Tesla/copy | covered | 2 | 5 | None |
| deadline editing | covered | 9 | 18 | None |
| high precision coordinates | covered | 3 | 4 | None |
| quick nav mobile-only | covered | 1 | 5 | None |
| supervisor worker-like access | covered | 5 | 11 | None |
| media delete only Andrey/Sergey | covered | 4 | 7 | None |
| file upload/open/download | covered | 7 | 22 | None |
| Jarvis confirmation and real id success | covered | 9 | 16 | None |
| archive/trash separation | covered | 4 | 15 | None |
| payroll untouched | covered | 3 | 5 | None |
| GPS/shift untouched | covered | 24 | 75 | None |
